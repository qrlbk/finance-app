...
function formatAmountWithSign(amount: number, type: string) {
...
export function QuickStats({ transactions, loading }: QuickStatsProps) {
  ...
  const formatTxDate = (dateStr: string) => {
    ...
    if (date.toDateString() === today.toDateString()) return t("quickStats.today");
    if (date.toDateString() === yesterday.toDateString()) return t("quickStats.yesterday");
    return formatDate(date, undefined, { day: "2-digit", month: "short" });
  };
  ...
  return (
    <div className="p-6 rounded-xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 shadow-sm dark:shadow-none animate-slide-up">
      <div className="flex justify-between items-center mb-5">
        <h4 className="font-medium text-zinc-900 dark:text-zinc-100">{t("quickStats.recentTransactions")}</h4>
        <Link
          to="/transactions"
        ...
        >
          {t("quickStats.all")} <ArrowRight size={14} />
        </Link>
      </div>
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="font-medium text-zinc-900 dark:text-zinc-100 truncate">
                  {tx.category_name ?? tx.note ?? t("quickStats.noCategory")}
                </p>
                <p className="text-sm text-zinc-500 dark:text-zinc-400">
                  {formatTxDate(tx.date)} · {tx.account_name}
                </p>
              </div>
              <span
              ...
              >
                {formatAmountWithSign(tx.amount, tx.transaction_type)} ₸
              </span>
            </div>
          );
        })}
      ...
      {recent.length === 0 && (
        <div className="text-center py-8">
          ...
          <p className="text-zinc-500 dark:text-zinc-400">{t("quickStats.noTransactions")}</p>
          <Link
            to="/transactions"
