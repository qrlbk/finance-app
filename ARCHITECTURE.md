...
- [x] Переводы между счетами, страница «Переводы»
- [x] Dashboard с аналитикой
- [x] Отчёты (Pie Chart, Bar Chart)
- [x] Тёмная/светлая тема
- [x] Резервное копирование (с проверкой ключа SQLCipher при восстановлении)
- [x] ML предсказание категорий
- [x] ML детекция аномалий
- [x] ML прогноз расходов
- [x] Автоплатежи (recurring) при старте приложения и по кнопке
- [x] Бюджеты по категориям с периодами (неделя/месяц/год), алерты в Header
- [x] Конвертация валют: базовая валюта, курсы, итоги в базовой валюте
- [x] Иерархия категорий (parent_id) в UI
- [x] Экспорт CSV/JSON/XLSX с фильтрами по счёту и категории
- [x] Импорт CSV/JSON/XLSX
- [x] Пагинация транзакций («Показать ещё»)
...
|---------|-----------|----------|
| Multi-profile | Low | Несколько пользователей (профилей) |
| Charts improvements | Medium | Расширенные графики и диаграммы |
| Notifications | Medium | Уведомления о событиях (превышение бюджета и др.) |
